...
export function Toast({ visible, className, ...other }: ToastProps) {
  return (
    <div
      className={cn(
...
Toast.Icon = function ToastIcon({ name, className, ...other }: ToastIconProps) {
  ...
  return (
    <div className="flex h-20 w-20 items-center justify-center rounded-full bg-slate-100">
      <Icon className={cn("h-10 w-10", className)} {...other} />
    </div>
  );
};
...
Toast.Title = function ToastTitle({
  ...
  return <p className={cn("text-sm font-medium", className)} {...other} />;
};
...
Toast.Description = function ToastDescription({
  ...
  return <p className={cn("text-sm opacity-80", className)} {...other} />;
};
